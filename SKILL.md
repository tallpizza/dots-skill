...
- Prefer API key auth for Dots HTTP API work: `Authorization: Bearer sk_{public_id}_{secret}`.
- `GET /api/workspaces`, `GET /api/workspaces/:ws/spaces`, `POST/PATCH/DELETE /api/workspaces/:ws/spaces...`, and `GET/POST/PATCH/DELETE .../:sp/labels...` can be called with an API key.
- These still require the appropriate workspace admin permission where applicable.
- If an API key call returns `401` or `403`, say that plainly and stop.
